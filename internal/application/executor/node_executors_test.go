...
// TestStartNodeExecutor tests the StartNodeExecutor
func TestStartNodeExecutor(t *testing.T) {
	t.Run("Execute returns empty map", func(t *testing.T) {
		executor := NewStartNodeExecutor()
		node := createMockNode(domain.NodeTypeStart, map[string]any{})
...
// TestEndNodeExecutor tests the EndNodeExecutor
func TestEndNodeExecutor(t *testing.T) {
	t.Run("Returns all variables when no output_keys specified", func(t *testing.T) {
		executor := NewEndNodeExecutor()
		node := createMockNode(domain.NodeTypeEnd, map[string]any{})
		variables := domain.NewVariableSet(nil)

		_ = variables.Set("key1", "value1")
		_ = variables.Set("key2", 42)
	...
	t.Run("Returns only specified output_keys", func(t *testing.T) {
		executor := NewEndNodeExecutor()
		config := map[string]any{
			"output_keys": []interface{}{"key1"},
		}
		node := createMockNode(domain.NodeTypeEnd, config)
		variables := domain.NewVariableSet(nil)

		_ = variables.Set("key1", "value1")
		_ = variables.Set("key2", "value2")
	...
	t.Run("Handles non-existent keys gracefully", func(t *testing.T) {
		executor := NewEndNodeExecutor()
		config := map[string]any{
			"output_keys": []interface{}{"nonexistent"},
		}
		node := createMockNode(domain.NodeTypeEnd, config)
		variables := domain.NewVariableSet(nil)
