...
const maxWorkflowSnapshotSize = 1_048_576

func (h *ExecutionHandlers) HandleRunEphemeralExecution(c *gin.Context) {
	var req struct {
	...
	if err != nil {
		h.logger.Error("Failed to start ephemeral execution", "error", err, "mode", req.Mode, "request_id", GetRequestID(c))
		respondAPIErrorWithRequestID(c, TranslateError(err))
		return
	}
	...
	h.logger.Info("Ephemeral execution started", "execution_id", execution.ID, "mode", req.Mode, "request_id", GetRequestID(c))
