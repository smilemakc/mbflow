...
CREATE TABLE mbflow_resource_rental_key (
    ...
    created_by UUID REFERENCES mbflow_users(id) ON DELETE SET NULL,
    provisioner_type VARCHAR(50) NOT NULL DEFAULT 'manual',

    -- Constraints
    CONSTRAINT mbflow_rental_key_provider_check
        CHECK (provider IN ('openai', 'anthropic', 'google_ai')),
    CONSTRAINT mbflow_rental_key_provisioner_check
        CHECK (provisioner_type IN ('manual', 'auto_openai', 'auto_anthropic', 'auto_google'))
);

-- Usage log table for detailed tracking (multimodal)
CREATE TABLE mbflow_rental_key_usage (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    rental_key_id UUID NOT NULL REFERENCES mbflow_resources(id) ON DELETE CASCADE,
    ...
    CONSTRAINT mbflow_rental_key_usage_status_check
        CHECK (status IN ('success', 'failed', 'rate_limited'))
);

-- Indexes for resource_rental_key
CREATE INDEX idx_mbflow_rental_key_provider ON mbflow_resource_rental_key(provider);
CREATE INDEX idx_mbflow_rental_key_pricing_plan ON mbflow_resource_rental_key(pricing_plan_id);
CREATE INDEX idx_mbflow_rental_key_created_by ON mbflow_resource_rental_key(created_by);
CREATE INDEX idx_mbflow_rental_key_last_used ON mbflow_resource_rental_key(last_used_at DESC NULLS LAST);

-- Indexes for rental_key_usage
CREATE INDEX idx_mbflow_rental_key_usage_key_time ON mbflow_rental_key_usage(rental_key_id, created_at DESC);
CREATE INDEX idx_mbflow_rental_key_usage_execution ON mbflow_rental_key_usage(execution_id) WHERE execution_id IS NOT NULL;
CREATE INDEX idx_mbflow_rental_key_usage_workflow ON mbflow_rental_key_usage(workflow_id) WHERE workflow_id IS NOT NULL;
CREATE INDEX idx_mbflow_rental_key_usage_created ON mbflow_rental_key_usage(created_at DESC);
CREATE INDEX idx_mbflow_rental_key_usage_model ON mbflow_rental_key_usage(model);
CREATE INDEX idx_mbflow_rental_key_usage_status ON mbflow_rental_key_usage(status) WHERE status != 'success';
...
-- Comments
COMMENT ON TABLE mbflow_resource_rental_key IS 'Rental API keys for LLM providers - managed by platform, never exposed to users';
COMMENT ON COLUMN mbflow_resource_rental_key.encrypted_api_key IS 'AES-256-GCM encrypted API key - NEVER exposed via any endpoint';
COMMENT ON COLUMN mbflow_resource_rental_key.provider_config IS 'Provider-specific configuration (org_id, base_url, etc.)';
COMMENT ON COLUMN mbflow_resource_rental_key.provisioner_type IS 'How the key was provisioned: manual by admin or auto by system';

COMMENT ON TABLE mbflow_rental_key_usage IS 'Detailed multimodal usage logs for billing and analytics';
COMMENT ON COLUMN mbflow_rental_key_usage.total_tokens IS 'Computed sum of all token types for this usage record';

-- Insert default pricing plans for rental_key
INSERT INTO mbflow_pricing_plans (id, resource_type, name, description, price_per_unit, unit, billing_period, pricing_model, is_free, is_active)
VALUES
