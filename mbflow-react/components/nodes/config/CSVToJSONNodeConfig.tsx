    ...
    return (
        <div className="space-y-6">
            ...
            <div
                ...
                <div>
                    <h3 className="font-semibold text-slate-900 dark:text-white text-sm">{t.nodeConfig.csvToJson.title}</h3>
                    <p className="text-xs text-slate-600 dark:text-slate-300 mt-0.5">
                        {t.nodeConfig.csvToJson.description}
                    </p>
                </div>
            </div>
            ...
            <div className="space-y-3">
                <label className="block">
                    <span className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2 block">
                        {t.nodeConfig.csvToJson.delimiter}
                    </span>
                    <select
                    ...
                    >
                        <option value=",">{t.nodeConfig.csvToJson.delimiterComma}</option>
                        <option value=";">{t.nodeConfig.csvToJson.delimiterSemicolon}</option>
                        <option value="\t">{t.nodeConfig.csvToJson.delimiterTab}</option>
                        <option value="|">{t.nodeConfig.csvToJson.delimiterPipe}</option>
                    </select>
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                        {t.nodeConfig.csvToJson.delimiterHint}
                    </p>
                </label>
            </div>
            ...
            <div className="space-y-3">
                <label className="block">
                    <span className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2 block">
                        {t.nodeConfig.csvToJson.inputKey}
                    </span>
                    <VariableAutocomplete
                        value={safeConfig.input_key}
                        onChange={handleInputKeyChange}
                        placeholder={t.nodeConfig.csvToJson.inputKeyPlaceholder}
                        className="w-full px-3 py-2 bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-700 rounded-lg text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-cyan-500 text-sm"
                    />
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                        {t.nodeConfig.csvToJson.inputKeyHint}
                    </p>
                </label>
            </div>
            ...
            <div className="space-y-3">
                <span className="text-sm font-semibold text-slate-700 dark:text-slate-300 block">
                    {t.nodeConfig.csvToJson.headersConfig}
                </span>
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    <span className="text-sm text-slate-700 dark:text-slate-300">
                        {t.nodeConfig.csvToJson.hasHeader}
                    </span>
                </label>
                    ...
                    <div className="space-y-2 pl-6">
                        <div className="flex items-center justify-between">
                            <span className="text-xs font-medium text-slate-600 dark:text-slate-400">
                                {t.nodeConfig.csvToJson.customHeaders}
                            </span>
                            <Button
                                type="button"
                                ...
                                iconPosition="left"
                            >
                                {t.nodeConfig.csvToJson.addHeader}
                            </Button>
                        </div>
                        ...
                        {safeConfig.custom_headers && safeConfig.custom_headers.length > 0 ? (
                                ...
                                {safeConfig.custom_headers.map((header, index) => (
                                    <div key={index} className="flex items-center gap-2">
                                        <input
                                            ...
                                            placeholder={t.nodeConfig.csvToJson.headerPlaceholder}
                            ...
                            <p className="text-xs text-slate-500 dark:text-slate-400 italic">
                                {t.nodeConfig.csvToJson.noCustomHeaders}
                            </p>
                        )}
                    </div>
                )}
            ...
            <div className="space-y-3">
                <span className="text-sm font-semibold text-slate-700 dark:text-slate-300 block">
                    {t.nodeConfig.csvToJson.processingOptions}
                </span>
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    />
                    <span className="text-sm text-slate-700 dark:text-slate-300">
                        {t.nodeConfig.csvToJson.trimSpaces}
                    </span>
                </label>
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    />
                    <span className="text-sm text-slate-700 dark:text-slate-300">
                        {t.nodeConfig.csvToJson.skipEmptyRows}
                    </span>
                </label>
            </div>
        </div>
