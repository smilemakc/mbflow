    ...
    return (
        <div className="space-y-6">
            ...
            <div
                ...
                <div>
                    <h3 className="font-semibold text-slate-900 dark:text-white text-sm">{t.nodeConfig.rssParser.title}</h3>
                    <p className="text-xs text-slate-600 dark:text-slate-300 mt-0.5">
                        {t.nodeConfig.rssParser.description}
                    </p>
                </div>
            </div>
            ...
            <div className="space-y-3">
                <label className="block">
                    <span className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2 block">
                        {t.nodeConfig.rssParser.url} <span className="text-red-500">{t.nodeConfig.required}</span>
                    </span>
                    <VariableAutocomplete
                        value={safeConfig.url}
                        onChange={handleUrlChange}
                        placeholder={t.nodeConfig.rssParser.urlPlaceholder}
                        type="input"
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                        {t.nodeConfig.rssParser.urlHint}
                    </p>
                </label>
            </div>
            ...
            <div className="space-y-3">
                <label className="block">
                    <span className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2 block">
                        {t.nodeConfig.rssParser.maxItems} <span className="font-normal text-slate-500 dark:text-slate-400">{t.nodeConfig.optional}</span>
                    </span>
                    <input
                        type="number"
                        min="0"
                        ...
                        placeholder={t.nodeConfig.rssParser.maxItemsPlaceholder}
                        className="w-full px-3 py-2 bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-700 rounded-lg text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-orange-500 dark:focus:ring-orange-400 text-sm"
                    />
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                        {t.nodeConfig.rssParser.maxItemsHint}
                    </p>
                </label>
            </div>
            ...
            <div className="space-y-3">
                <span className="text-sm font-semibold text-slate-700 dark:text-slate-300 block">
                    {t.nodeConfig.rssParser.options}
                </span>
                ...
                <label className="flex items-center gap-3 cursor-pointer group">
                    ...
                    <div>
                        <span className="text-sm text-slate-700 dark:text-slate-300 group-hover:text-slate-900 dark:group-hover:text-white">
                            {t.nodeConfig.rssParser.includeContent}
                        </span>
                        <p className="text-xs text-slate-500 dark:text-slate-400">
                            {t.nodeConfig.rssParser.includeContentHint}
                        </p>
                    </div>
                </label>
            </div>
            ...
            <div
                ...
                <div className="flex items-start gap-3">
                    ...
                    <div>
                        <h4 className="text-xs font-bold text-blue-900 dark:text-blue-100 mb-2">{t.nodeConfig.rssParser.supportedFormats}</h4>
                        ...
                        <h4 className="text-xs font-bold text-blue-900 dark:text-blue-100 mb-2 mt-3">{t.nodeConfig.rssParser.autoDetection}</h4>
                        <p className="text-xs text-slate-700 dark:text-slate-300">
                            {t.nodeConfig.rssParser.autoDetectionHint}
                        </p>
                        <h4 className="text-xs font-bold text-blue-900 dark:text-blue-100 mb-2 mt-3">{t.nodeConfig.rssParser.useCases}</h4>
                        <ul className="text-xs text-slate-700 dark:text-slate-300 space-y-1 list-disc pl-4">
            ...
            <div
                ...
                <p className="text-xs text-slate-600 dark:text-slate-400 font-medium mb-2">{t.nodeConfig.rssParser.outputFields}</p>
